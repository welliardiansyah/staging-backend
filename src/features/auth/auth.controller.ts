...
import { LoginPayloadEmail, LoginPayloadPhone } from './payloads/login.payload';
import {
...
export class AuthController {
  ...
  async email(
    @Body() credentials: LoginPayloadEmail,
  ): Promise<ResponseInterface<User>> {
    try {
      const response = await this.authService.email(credentials);
  ...
  @UsePipes(new ValidationPipe())
  async phone(
    @Body() credentials: LoginPayloadPhone,
  ): Promise<ResponseInterface<User>> {
    try {
      const response = await this.authService.phone(credentials);
      return new ResponseSuccess('LOGIN.SUCCESS', response);
    } catch (e) {
      return new ResponseError('LOGIN.ERROR', e.message);
    }
  ...
  async sendEmailForgotPassword(@Param() params: any): Promise<any> {
    try {
      const emailForgotPasswordSent: boolean =
  ...
  @Get('reset-password')
  @HttpCode(HttpStatus.OK)
  async getEmailForResetPassword(@Query() params: ForgotPasswordParamPayload) {
    try {
