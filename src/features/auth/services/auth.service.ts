...
export class AuthService {
  ...
  async register(
    ...
    await this.consentRegistryService.saveUserConsent(user.email, user.phone);
  ...
  async email(credentials: LoginEmailCredentials): Promise<User | any> {
    const user: User | any = await this.usersService.findEmailAndPasswordById(
      credentials.email,
  ...
  async phone(credentials: LoginPhoneCredentials): Promise<User | any> {
    const user: User | any = await this.usersService.findPhoneAndPasswordById(
      credentials.phone,
      credentials.password,
    );

    if (!user) throw Error('LOGIN.USER_NOT_FOUND');
    if (!user.auth.phone.valid) throw Error('LOGIN.EMAIL_NOT_VERIFIED');
    const access_token = await this.createToken(user.phone, user.id);
    return { user: this.formatUser(user), access_token };
  }
  ...
  async getEmailByForgotPasswordToken(
  ...
  async validateEmail(signedUser: LoginEmailCredentials): Promise<User> {
    const user = await this.usersService.findByEmail(signedUser.email);
    return user || null;
  }

  async validatePhone(signedUser: LoginPhoneCredentials): Promise<User> {
    const user = await this.usersService.findByPhone(signedUser.phone);
    return user || null;
  }
}
